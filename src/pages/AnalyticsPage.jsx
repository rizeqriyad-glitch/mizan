  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '2rem' }} className="analytics-grid">
        {[
          { label: isAr ? 'المهام المكتملة' : 'Tasks Completed', value: totalDone, icon: glyph('tasks'), color: 'var(--mizan-cyan)', dim: 'rgba(23, 102, 181,0.1)' },
          { label: t('streak'), value: `${stats.streak || 0} ${isAr ? 'يوم' : 'days'}`, icon: glyph('streak'), color: 'var(--mizan-purple)', dim: 'rgba(29, 127, 226,0.1)' },
          { label: t('points'), value: stats.points || 0, icon: glyph('points'), color: 'var(--mizan-purple)', dim: 'rgba(29, 127, 226,0.1)' },
        ].map((card, i) => (
          <motion.div
            key={card.label}
                    ...
                    return (
                      <td key={d.key} style={{ textAlign: 'center', padding: '0.5rem 0.25rem' }}>
                        <div style={{
                          width: 28, height: 28,
                          borderRadius: '8px',
                          background: done ? 'rgba(23, 102, 181,0.1)' : 'rgba(255,255,255,0.05)',
