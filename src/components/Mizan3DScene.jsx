...
// Particle System Class (moved outside component for reusability)
...
export default function Mizan3DScene({ completedItems = [], selectedDate = null }) {
  // completedItems is now a prop, not from context
  const canvasRef = useRef(null)
  const lastCount = useRef(completedItems.length)
  ...
  useEffect(() => {
    ...
    // Beam with custom shader for ripple effect
    const beamMaterial = glassMat.clone(); // Clone material to apply custom shader only to beam
    const beam = new THREE.Mesh(new THREE.BoxGeometry(5.5, 0.06, 0.06), beamMaterial)
    beamGroup.add(beam)

    // Define uniforms for the beam's custom shader
    const beamPulseUniforms = {
      u_pulsePositions: { value: new Array(10).fill(new THREE.Vector3()) }, // Max 10 pulses
      u_pulseStrengths: { value: new Array(10).fill(0.0) },
      u_pulseRadii: { value: new Array(10).fill(0.0) },
      u_pulseDurations: { value: new Array(10).fill(0.0) },
      u_pulseStartTimes: { value: new Array(10).fill(0.0) },
      u_numPulses: { value: 0 },
    ...
    beamMaterial.onBeforeCompile = (shader) => {
      // Inject uniforms
      Object.assign(shader.uniforms, beamPulseUniforms);
      ...
      // Inject vertex shader code to displace vertices
      shader.vertexShader = shader.vertexShader.replace(
        ...
        `
          ...
          vec3 displacedPosition = transformed;

          for (int i = 0; i < MAX_PULSES; i++) {
            if (i < u_numPulses) {
              float elapsed = u_currentTime - u_pulseStartTimes[i];
              if (elapsed > 0.0 && elapsed < u_pulseDurations[i]) {
                vec3 pulseWorldPos = u_pulsePositions[i]; // These are already in world space
                float pulseStrength = u_pulseStrengths[i];
                float pulseRadius = u_pulseRadii[i];
                float pulseDuration = u_pulseDurations[i];

                float distance = length(worldPosition - pulseWorldPos);
                float normalizedTime = elapsed / pulseDuration;

                float waveFront = normalizedTime * pulseRadius;
                float waveThickness = 0.3; // Thickness of the visible ripple

                float influence = 0.0;
                if (distance > waveFront - waveThickness && distance < waveFront + waveThickness) {
                  influence = 1.0 - abs(distance - waveFront) / waveThickness;
                  influence = pow(influence, 2.0); // Sharpen the peak
                }
                float ageFade = 1.0 - normalizedTime;
                displacedPosition += normal * influence * ageFade * pulseStrength * 0.2; // Scale displacement
              }
            }
          }
  ...
  useEffect(() => {
    if (completedItems.length > lastCount.current && sceneRef.current) {
      const newItems = completedItems.slice(lastCount.current)
    ...
    lastCount.current = completedItems.length
  }, [completedItems])
